...
def build_all_outputs(all_items=None, rules_config=None, domain=""):
    ...
    # Force Worker proxy - fixes US IP block
    if not domain:
        domain = "https://chilltv.chungchung.online"
    print(f"[BUILDER] domain={domain}", flush=True)
    ...
    if hot_tv_feed_items:
        ...
        if not hot_tv_dataset.get("list"):
            print(f"[FALLBACK] TV using direct, domain={domain!r}", flush=True)

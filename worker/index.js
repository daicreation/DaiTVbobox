  ...
  async fetch(request, env, ctx) {
    ...
    if (path === '/' || path === '/api') {
      return new Response(JSON.stringify({
        sites: [{ key: "Chill_AI_TV", name: "🧊 Chill-AI-TV", type: 1,
    ...
    if (path === '/api.php/provide/vod') {
      ...
      return proxy('https://bfzyapi.com/api.php/provide/vod' + url.search);
    }
...
async function proxy(target) {
  try {
    const r = await fetch(target, { headers: { 'User-Agent':'ChillAITV/1.0' }, signal: AbortSignal.timeout(8000) });
